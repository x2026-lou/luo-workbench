...
const medicalData = [
  ...
  { subject: '药理学', frame: [
    ...
    wrong: ['首过效应误用于注射剂；治疗指数越大越安全记反；阿托品"抑腺/扩瞳"混淆'],
    hard: ['受体量与效关系、肝药酶诱导/抑制对药动学影响、抗菌药选择压力与耐药'],
    mnemonic: ['"首过肝里代谢少，舌下直肠绕道好；指数大更安全，半减期定间隔"'] },
  { subject: '医学免疫学', frame: [
    ...
    wrong: ['IgG与IgM"最早/最高"记反；Ⅰ型与Ⅳ型机制混淆；补体三条途径启动混淆'],
    hard: ['T/B细胞活化双信号、阴性/阳性选择、自身免疫病机制'],
    mnemonic: ['"IgG最高过胎盘，IgM最早五聚体；一速四迟记分明"'] },
  { subject: '医学微生物学', frame: [
    ...
    wrong: ['革兰+ / −颜色记反；内/外毒素来源混淆；HBV"大三阳"指标组合记错'],
    hard: ['病毒复制周期、细菌耐药机制、标本采集与培养鉴定'],
    mnemonic: ['"革兰阳紫阴红，内毒脂多糖；乙肝看表面，保护抗HBs"'] },
  { subject: '系统解剖学', frame: [
    ...
    wrong: ['胸骨角平对肋序记错；12对脑神经序号与名称混淆；门静脉与腔静脉混淆'],
    hard: ['局部解剖三角与危险区、神经支配节段性、血管变异'],
    mnemonic: ['"一二嗅视动滑车，三叉外展面前庭；舌咽迷走副舌下，十二神经记分明"'] },
  { subject: '病理生理学', frame: [
    ...
    wrong: ['高低渗脱水"失什么多"记反；四型缺氧对应疾病混淆；DIC"凝血与出血并存"理解错'],
    hard: ['酸碱代偿预计公式、休克分期循环改变、电解质紊乱心电图'],
    mnemonic: ['"低渗失钠血钠低，高渗失水口渴急；四型缺氧分清楚，DIC凝血出血俱"'] },
  { subject: '预防医学与流行病学', frame: [
    ...
    wrong: ['发病率与患病率混淆；RR与OR适用研究类型记反；三级预防"一级"含义混淆'],
    hard: ['统计假设检验、偏倚(选择/信息/混杂)控制、样本量估算'],
    mnemonic: ['"一防病因二早诊，三防残障促康复；发病新患患病全，队列RR病例OR"'] }
];
...
function compareTable(rows) {
  ...
  return `<table class="med-compare"><thead><tr><th>对比项</th><th>A</th><th>B</th></tr></thead><tbody>${
    rows.map(r => `<tr><td class="cmp-n">${esc(r.n)}</td><td>${esc(r.a)}</td><td>${esc(r.b)}</td></tr>`).join('')
  }</tbody></table>`;
}
